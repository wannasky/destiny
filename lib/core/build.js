...
const compile = async (file, config, callback) => {
    ...
    let distFilePath = path.join(config.cwd, config.dist, sourceFileRelative);
    ...
    if(['.ts', '.js', '.jsx'].includes(extname)){
        ...
        const minify = typeof config.options.minify === "boolean" ? config.options.minify : (config.options.minify.script ? config.options.minify.script : false);
        try {
        ...
        if(config.options.styleSourceMap){
            ...
            const minify = typeof config.options.minify === "boolean" ? config.options.minify : (config.options.minify.style ? config.options.minify.style : false);
        ...
        // 不支持此文件类型的解析 直接copy
        distFilePath = path.join(config.cwd, config.dist, sourceFileRelative);
        fileUtil.copyFile(filename, distFilePath)
    }
    if(result && result.code){
        ...
        if(config.options[sourceMapKey] && result.map) {
            ...
            outputFile(result.map, sourceMapFilePath);
        }
