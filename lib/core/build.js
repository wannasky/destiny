...
class Dest extends EventEmitter {
...
const build = async (config, callback) => {
    ...
    const dest = new Dest(config);

    if (config.memory) {
        fileUtil.setMemoryFs(new MemoryFileSystem());
        dest.setMemoryFs(fileUtil.mfs);
    }
    ...
    config.files.forEach(item => {
        compile(item, config, () => {
            ...
            if (index === 0 && config.watch) {
                ...
                addWatch(watchFiles, config, dest);
                if (callback) {
                    callback(dest);
                }
            }
...
const addWatch = (files, config, dest) => {
        ...
        this.on('changed', (file) => {
            ...
            if (depsIndex[file].length) {
                ...
                depsIndex[file].forEach(item => {
                    compile(item, config, () => {
                        index++;
                        if(index === depsIndex[file].length) {
                            dest.emit('compiled', depsIndex[file]);
                        }
                    });
                });
            }
